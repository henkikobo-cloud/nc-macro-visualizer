...
function buildFlowGraph(programLines, controls) {
  ...
  for (let index = 0; index < mainNodes.length - 1; index += 1) {
    ...
    if (!control) {
      edges.push({ from: current.id, to: next.id, label: current.id === "START" ? "" : "次へ" });
      continue;
    }
    ...
    if (control.kind === "IF_THEN") {
      ...
      nodes.push(thenNode);
      edges.push({ from: current.id, to: thenNode.id, label: "はい / 実行", route: "branch" });
      edges.push({ from: thenNode.id, to: next.id, label: "合流", route: "merge" });
      edges.push({ from: current.id, to: next.id, label: "いいえ / 次へ" });
      continue;
    }

    if (control.kind === "IF_GOTO" && nodeById.has(control.target)) {
      edges.push({ from: current.id, to: control.target, label: `はい -> ${control.target}`, route: "jump" });
      edges.push({ from: current.id, to: next.id, label: "いいえ / 次へ" });
      continue;
    }

    if (control.kind === "GOTO" && nodeById.has(control.target)) {
      edges.push({ from: current.id, to: control.target, label: `ジャンプ -> ${control.target}`, route: "jump" });
      continue;
    }

    edges.push({ from: current.id, to: next.id, label: "next" });
  }
